...
func (c *client) watchNamespaceEvents(ctx context.Context, wg *sync.WaitGroup) error {
	watcher, err := c.clientset.CoreV1().Namespaces().Watch(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("failed to create watch interface: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to receive a namespace list: %s", err)
	}
...
func (c *client) watchSecretEvents(ctx context.Context, wg *sync.WaitGroup) error {
	...
	secrets, err := c.clientset.Secrets(c.origin).List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("failed to receive a secret list: %s", err)
	}
